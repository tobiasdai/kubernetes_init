...
KUBECONF=/home/$USER/kubeadm.conf # 文件地址, 改成你需要的路径（默认为当前USER的主目录下）
...
apply_flannel() {
  ...
  sed -i "s,quay\.io/coreos,registry.registry.cn-hangzhou.aliyuncs.com/acs,g" /tmp/flannel.yaml # use google-containers, but there is no 9.1 version yet
